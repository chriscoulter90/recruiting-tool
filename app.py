...
st.set_page_config(page_title="Coulter Recruiting v1.10", page_icon="🏈", layout="wide")
...
def load_lookup_v1_10():
    ...
    for _, row in df.iterrows():
        ...
        if full_name:
            ...
            s_key = normalize_text_v1_10(raw_school)
            n_key = normalize_text_v1_10(full_name)
            l_key = normalize_text_v1_10(last)
...
# *** V1.10: Cache Clear ***
if "master_data_v1_10" not in st.session_state:
    st.session_state["master_data_v1_10"] = load_lookup_v1_10()
master_lookup, global_name_lookup, lastname_lookup, db_status = st.session_state["master_data_v1_10"]
...
def determine_role_v1_10(title, bio_text):
...
def parse_header_v1_10(bio):
    ...
    extracted['Role'] = determine_role_v1_10(extracted['Title'], bio)
...
# --- 4. SEARCH LOGIC ---
st.markdown('<p class="instruction-text">Type in keywords to search college football webpage bios.<br>Put a comma between keywords for multiple searches (e.g., "Tallahassee, Recruiting").</p>', unsafe_allow_html=True)
            ...
            try:
                ...
                if mask.any():
                    ...
                    for idx, row in matches.iterrows():
                        meta = parse_header_v1_10(row['Full_Bio'])
                        ...
                        s_key = normalize_text_v1_10(meta['School'])
                        n_key = normalize_text_v1_10(name)
                        l_key = normalize_text_v1_10(meta['Last'])
