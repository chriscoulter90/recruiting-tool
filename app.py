...
try:
    ...
    def process_search_streaming(master_lookup, name_lookup, keywords):
        ...
        if not chunk_files:
            st.error("❌ CRITICAL ERROR: No 'chunk_*.csv' files found in the main folder!")
            return pd.DataFrame()
        ...
        for i, filename in enumerate(chunk_files):
            ...
            try:
                ...
                st.write(f"⚠️ Warning: Issue reading {filename}: {e}")
                continue
    ...
    if chunk_count == 0:
        st.error("❌ No 'chunk_*.csv' files found! Please upload them to your GitHub repository.")
    else:
    ...
    st.error(f"Here is exactly what went wrong: {e}")
    st.info("Please copy the error message above and paste it in the chat.")
